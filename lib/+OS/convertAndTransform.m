function [ varargout ] = convertAndTransform(x,y,varargin)
    ...
    %              'struct' - containing fields 'Easting' & 'Northing' or 'Longitude' & 'Latitude', as appropriate
    %
    ...
    %
    % EXAMPLES:
    %    [x,y]=OS.convertAndTransform(-1,52,'lut',lut,'csin','GPS','csout','NG','from','LL','to','EN') %will convert lat/longs from GPS to Eastings/Northings on the National Grid
    %    [x,y]=OS.convertAndTransform(-1,52) % x = easting, y = northing. Same results as above (assumptions made about input values)
    %    x=OS.convertAndTransform(-1,52) % x is now a 1x2 vector where x(1) = easting,  x(2) = northing
    %    x=OS.convertAndTransform(-1,52,'output','struct') % x is struct with fields "easting" , "northing"
    %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Terminology:
    %
    ...
    %       e=651409.903;
    %       n=313177.270;
    %       ll=OS.convertAndTransform(e,n,'csin','NG','csout','NG','warning',false);
    ...
    %       OS.convertAndTransform(300000,600000)
    %    produces:
    ...
    for v = 1:length(varargin)
      switch varargin{v}
        case 'from'
          options.from = varargin{v + 1};
        case 'to'
          options.to = varargin{v + 1};
        case 'csin'
          options.csin = varargin{v + 1};
        case 'csout'
          options.csout = varargin{v + 1};
        case 'transformationType'
          options.transformationType = varargin{v + 1};
        case 'lut'
          options.lut = varargin{v + 1};
        case 'autoload'
          options.autoload = varargin{v + 1};
        case 'output'
          options.output = varargin{v + 1};
        case 'verbose'
          options.verbose = varargin{v + 1};
        case 'warning'
          options.warning = varargin{v + 1};
      end
    end

    % Lots of sub-functions in this function. We use a global variable so we don't need to keep passing it around
    global verbose; 
    ...
    if(~strcmp(options.csin,options.csout)) % input coordinate system is different from output?
        ...
        else procedureSteps(length(procedureSteps)+1)={'A2'}; % Input is OSGB36 and output =GPS (WGS84)
            if verbose
    ...
    if any(cell2mat(strfind(procedureSteps,'A')))
        ...
        if(isempty(find(strcmp(options.transformationType,'Helmert'), 1))) % ie transformation type is not simply the Helmert transform...
            ...
            elseif options.autoload % try importing data to workspace
                try
                    ...
                    fprintf('Please ensure you have copy of file ''OSTN02_OSGM02_GB.mat''\n')
                    fprintf('Aborting...\n')
                    return
                end
            end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % We also need the projection constants. Using the OSGB National Grid
        ...
        if(~isnan(xi)&&~isnan(yi))
            ...
            for procno=1:length(procedureSteps)
                ...
                if ~isempty(regexp(step,'A', 'once')) %Step contains 'A' - it's a transformation!
                    ...
                    else error('Cant find relevant ellipsoid! Please use ''GPS'' or ''NG''Aborting...')
                    end
            ...
            if options.warning && abs(xi)>abs(yi)
                warning('OH:DEAR','OUTPUT |X| value > |Y| value !')
            end
